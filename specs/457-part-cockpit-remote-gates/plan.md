...
- **New MCP tools consumed** (bound by the cluster per generacy#1038, not registered by this ticket):
  - `cockpit_gate_status(gateId) → { gateId, status: 'open'|'answered' } | { gateId: null, status: 'absent' }` — exact match on `gateId`; used in the pre-draft check step 1 to detect the same-`gateId` reuse case.
  - `cockpit_gate_list({ issueRef, gateType }) → Array<{ gateId, generation, status, ... }>` — enumerates non-terminal gates for the `(issue, kind)` pair; used in step 2 to detect generation drift (per Q1=C rationale citing `query-schemas.ts:58-72` on the upstream side).
...
2. Call `cockpit_gate_status(gateId)` where `gateId = hash(issueRef, gateType, generation)` (the MCP tool derives the hash — the plugin passes semantic inputs, never a hand-built hash).
3. Branch on the return:
   - **`{ status: 'open' }` or `{ status: 'answered' }`** — skip the subagent; record the existing gate in `openGates` (creating a `GateRecord` with `openedAt` from the status return; may be earlier than the run's start on a takeover) with the returned `status` on the record; continue to next event. For `answered`, tick the sweep counter (see below). **The drafting subagent is NOT spawned.**
   - **`{ status: 'absent' }`** — run `cockpit_gate_list({ issueRef, gateType })`; branch again:
     - If the list returns a non-terminal gate at a DIFFERENT `generation` (generation drift, Q1=C) — call `cockpit_gate_ack(staleGateId, outcome: 'superseded', detail: 'generation drift — content changed since original draft (was g<old>, now g<new>)')`. Then continue to the current draft-then-open flow (subagent spawn → present gate → `cockpit_gate_open`) with the fresh generation. The stale gate transitions terminal cloud-side (per `handleGateOutcome`, generacy-cloud `services/api/src/services/relay/message-handler.ts:934-980`) so a subsequent status query reports `absent` and a subsequent list query filters it out.
     - If the list returns nothing (no gate anywhere for this `(issue, kind)`) — run the current draft-then-open flow unchanged.
