...
- [X] T001 [P] Add a changeset `.changeset/<name>.md` with a patch bump to
...
- [X] T002 [US1] Split D.12 step 1 into a lookup + three-way no-record branch in
...
- [X] T003 [US1] Document the `gateKey` parse rules inside the D.12 step 1 branch
...
- [X] T004 [US2] Add the two verbatim no-op ledger rows to the D.12 **Ledger line**
...
- [X] T005 [US2] Register both no-op outcome vocabularies in the enumeration sites so
...
- [X] T006 [US1] Update the D.12 **Payload shape** section (~auto.md:1087) to document
...
- [X] T007 [US3] Add a new `519-*` describe block to
...
- [X] T009 [US3] Run `pnpm test` in `packages/claude-plugin-cockpit` (`vitest run`).
