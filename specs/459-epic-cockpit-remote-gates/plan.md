...
The probe is inserted as an additional condition — the **third** condition of `auto` and an additional pre-flight assertion under explicit `ui` — with a two-phase ordering: items 1–2 evaluate at parse time (before the ledger directory is created); item 3 (the probe) is DEFERRED until AFTER the ledger header is written, so the probe's pass/fail ledger row can be safely appended without violating the header-first invariant (auto.md line 199):

1. Under **explicit `--gates=ui`**: at parse time, check all three UI-mode tools bound (`cockpit_gate_open`, `cockpit_gate_status`, `cockpit_gate_list`) — any absent → ledger-free hard-fail (unchanged in shape, extended in tool-set so a partial-deployment cluster does not slip through and hit an unbound `cockpit_gate_list` at the probe). After F4.6/F4.4 has bound the identity ref AND F4.7 has written the ledger header (or the equivalent header-write for Forms 1/2/3), issue the probe. Probe passes → write pass ledger row + proceed with `ResolvedGateMode = "ui"`. Probe fails → take the § step-1 probe fail path (write fail ledger row — safe because header exists — + print FR-013 template line + exit non-zero).
2. Under **`--gates=auto`**: at parse time, evaluate items 1 (all three UI-mode tools bound) AND 2 (cluster cloud-activated). Both YES → tentative `ResolvedGateMode = "ui pending probe"`. After F4.6/F4.4 has bound the identity ref AND the ledger header has been written, issue the probe. Probe passes → resolve to `ui`, write pass ledger row. Probe fails → resolve to `local`, write fail ledger row (safe because header exists). When EITHER of items 1–2 fails at parse time, short-circuit: resolve to `local` immediately with **no probe call and no probe ledger row** (per Q3 answer + FR-005).
...
Q3's option-C alternative ("short-circuit AND record a distinguishable ledger row like `probe-skipped: not-cloud-activated`") is mechanically unwritable because items 1–2 of the `--gates=auto` three-part check run BEFORE the ledger directory is created (parse-time evaluation); adding a ledger row there would require creating the directory earlier, which every existing `--gates=local` path assumes is not done. Item 3 (the probe) is different — it is DEFERRED until after the header write, so its pass/fail row is safe to append. The resolved mode is already visible in the `Auto run starting` line and the ledger header, so no ledger row is missing for the skip case.
...
1. **§ step 1 `--gates` resolution** — extend the `--gates=auto` two-part check to a three-item list with a two-phase ordering (items 1–2 at parse time; item 3 the probe, DEFERRED until after the ledger header write). Item 3 has the short-circuit rule stated verbatim ("issue item 3 ONLY when items 1 AND 2 both pass; otherwise resolve to `local` with NO probe call and NO probe ledger row"). Item 1's tool-binding check is extended from `cockpit_gate_open` alone to all three UI-mode tools (`cockpit_gate_open`, `cockpit_gate_status`, `cockpit_gate_list`), so a partial-deployment cluster does not slip through and hit an unbound `cockpit_gate_list` at the probe. Extend the explicit `--gates=ui` block to (a) extend the absence check to all three UI-mode tools and (b) add a pre-flight probe step, running AFTER the tool-binding check, AFTER F4.6/F4.4 has bound the identity ref, and AFTER F4.7 (or the equivalent header write for Forms 1/2/3), that hard-fails the run on any probe error.
2. **§ step 1 `Auto run starting` line** — add `probe-failed` as a new possible value of `<resolution reason>` in the `→ <resolution reason>` suffix (fires only under `--gates=auto` when the probe was the reason for resolving down to `local`); update the printed-position rule so the line follows the header (and any probe row), not precedes them.
3. **§ Ledger clause "pre-flight failures do not earn a row"** — narrow amendment: the probe row (`preflight · gate-query-probe · ok|error`) IS a ledger row, safe by construction because the probe is deferred until after the ledger header write (Forms 1/2: at line-199 header write; Form 3: at post-G.6 header write; Form 4: at F4.7). The § step-1 hard-fail path itself (missing any UI-mode tool under explicit `ui`; usage errors) remains ledger-free — those failures happen at parse time, BEFORE any ledger directory is created.
