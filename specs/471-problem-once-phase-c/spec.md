...
**Acceptance Criteria**:
- [ ] The adoption sweep's `cockpit_gate_list` calls (one per in-scope issue, per FR-001) carry no `runId` field.
...
### US5: One `cockpit_gate_list` failure does not sink the whole run

**As an** operator whose `/cockpit:auto` run against an N-child epic hits a transient `cockpit_gate_list` failure on one child,
**I want** the sweep to defer both adoption and drafting for that one child (writing a ledger row) and continue processing the other N-1 children,
**So that** a single child's transient error does not abort the whole run, and the deferred child's natural label-persistent event re-fires on the next natural wake without producing a duplicate inbox gate.

**Acceptance Criteria**:
- [ ] On a per-issue `cockpit_gate_list` error, zero `cockpit_gate_open` fires for that issue this pass.
- [ ] A ledger row is written naming the failing issue and the error class.
- [ ] Adoption and drafting complete normally for every other in-scope issue.
...
|----|-------------|----------|-------|
| FR-001 | On startup, before drafting any gate, the sweep MUST call `cockpit_gate_list({ issueRef: <ref>, gateType: <omitted> })` **exactly once per in-scope issue** — the tracking ref itself PLUS every in-scope child under it — enumerating every returned non-terminal row. For an epic-mode run against an epic with N in-scope children, that is **N+1 calls**. Runs after #469's pre-flight capability probe, before the sweep's synthetic-event pass and before any D.n Step-0 pre-draft check. | P1 | Resolved by Q2=A (2026-07-29). `cockpit_gate_list` filters by `issueRef`; the gates that matter (D.1 clarification, D.2 clarification-review, D.3 plan-review, D.4 tasks-review, D.7 implementation-review, D.11 manual-validation) are opened against **child** issues — a single call against the epic ref sees only D.6 and D.7-phase-complete gates on the epic body. No later "optimisation" back to one call. |
...
| FR-003 | An adopted `openGates` entry MUST carry the originating `runId` read from the list row's `runId` field. `cockpit_gate_ack` for that entry MUST target the originating `runId`, not the current run's. | P1 | The list row's `runId` field is guaranteed by generacy-cloud#892. `GateAckInputSchema` documents `runId` as accepted-and-ignored on the ack path — the ack targets an existing `gateId` — so superseding a prior-run gate needs nothing special. |
...
| FR-009 | The sweep MUST adopt **every** non-terminal row `cockpit_gate_list` returns for an in-scope issue into `openGates`, including rows whose `(gateType, generation)` does NOT match a natural gate the current run's sweep would draft. `dispatchClass` for such rows is derived from `(gateType, generation)` using the same mapping-table rule the current-run sweep uses. Rows for out-of-scope issues (not part of this run's tracking-ref sweep set) are still skipped. **Precedence:** FR-013 (generation-drift branch) is the more specific rule and wins where it applies — a row whose `gateType` matches a natural gate but whose `generation` differs is NOT adopted at the stale generation; it is superseded and redrafted per FR-013. | P1 | Resolved by Q1=A (2026-07-29). Narrower rules (adopt only matching rows) leave orphaned inbox entries — the exact symptom this spec eliminates — including a common case where a prior run opened `implementation-review` on a child that has since moved to a phase where this run would draft `manual-validation`. Adopting broadly is safe: an unanswered adopted `open` entry sits in `openGates` and does nothing; the escape hatch only ticks `answered` entries. |
| FR-010 | An adopted gate whose status is `answered` MUST be dispatched through the same answered-gate handling the current run uses for its own answered entries (the § "Answered-gate parked-forever escape hatch" block in `auto.md`). The `answeredGateSweepCounter[gateId]` for an adopted `answered` entry MUST be initialised to `1` at adopt time (matching the reuse-answered branch). **Documented limitation:** no MCP surface currently returns the answer document (`cockpit_gate_status` returns `{gateId, status}`; `cockpit_gate_list` returns `{gateId, gateType, generation, status, runId}`), so the current run structurally cannot consume a prior-run answer on its own. The answer is preserved *only if* D.12 redelivery fires; otherwise the escape hatch supersedes after 3 sweeps and re-derives from current labels — which either dispatches correctly (labels moved) or re-asks the operator (they did not). Any path that supersedes immediately guarantees the re-ask even when redelivery would have worked. See Follow-up. | P1 | Resolved by Q3=A (2026-07-29). |
...
| FR-013 | For a row whose `(issueRef, gateType)` matches a natural gate the current run's sweep would draft but whose `generation` differs, the sweep MUST mirror the live-path `generation-drift branch`: ack the prior-run gate `superseded` (targeting the prior-run's `runId` per FR-003) with the same detail message the live-path uses, then draft fresh at the current-run generation. This applies to `clarification`, `artifact-review`, `implementation-review`, and `manual-validation`. **Carve-out:** for `gateType: 'escalation'` the drift branch is DISABLED (per `auto.md` line 317; four dispatch rows share the one enum value and the wire carries no subtype discriminator — upstream generacy#1046); a prior-run `escalation` gate is adopted per FR-009 and left non-terminal. FR-013 takes precedence over FR-009 where it applies. | P1 | Resolved by Q4=A (2026-07-29). Adopting a gate at its prior `generation` would apply an operator verdict computed against **old content** to **current content** — the exact hazard the live-path drift branch exists to prevent. Keeping the sweep and the live path symmetric leaves one drift rule to reason about, not two that can diverge. |
| FR-014 | When `cockpit_gate_list` returns `status: 'error'` for an in-scope issue, the sweep MUST skip **both adoption and drafting** for that issue in this pass — no `cockpit_gate_open` fires for any natural gate on that issue. The sweep MUST write a ledger row naming the failing issue and the error class, and MUST continue processing the other in-scope issues. The run MUST NOT abort. The underlying label(s) that would trigger the drafted gate remain persistent, so the event re-fires on the main loop's next natural wake — matching `auto.md`'s existing sweep-time `cockpit_gate_open` failure pattern ("the specific gate's initiation is DEFERRED to the main loop's first natural wake … the underlying event WILL re-fire naturally because the label is persistent"). No additional retry layer is added at the playbook level; `cockpit_gate_list` already retries internally per `QUERY_RETRY_SCHEDULE` (3 attempts, ~5s backoff, ~20s worst case per `auto.md` probe section), so by the time the playbook sees `status: 'error'` the transient case has already been absorbed. | P1 | Resolved by Q5=D (2026-07-29). Aborting the run is too blunt (blast radius under Q2=A: N+1 issues); soft-fail-and-draft silently reintroduces the duplicate-inbox symptom the spec exists to remove; stacking a playbook-level retry on top of the tool-level retry adds ~35s per failing issue without buying anything. |
...
| SC-008 | Adoption call count = N+1 for epic with N children | An epic-mode run against an epic with N in-scope children fires exactly N+1 `cockpit_gate_list` calls on the adoption path (one per in-scope issue: epic itself + every child). | Log grep asserting the call count equals `1 + inScopeChildCount` on integration test runs. |
| SC-009 | Broad adoption of non-matching non-terminal rows | Given a seeded non-terminal gate whose `(gateType, generation)` does NOT match any natural gate the current run's sweep would draft (e.g. prior-run `implementation-review` on a child now in `manual-validation` phase), the sweep adopts the gate into `openGates` with a resolvable `dispatchClass` (FR-009). | Integration test: seed such a gate, assert `openGates` contains an entry with the seeded `gateId` and a non-null `dispatchClass`. |
| SC-010 | Generation-drift on adopted gate supersedes and redrafts | Given a seeded non-terminal gate whose `(issueRef, gateType)` matches a natural gate the current run would draft but whose `generation` differs, the sweep acks the prior-run gate `superseded` (targeting the prior-run's `runId`) and drafts one fresh gate at the current generation — the operator sees exactly one gate, not two (FR-013). | Integration test: seed a drift scenario for each of `clarification`, `artifact-review`, `implementation-review`, `manual-validation`; assert 1 ack + 1 open per scenario. |
| SC-011 | Escalation carve-out on adoption drift | Given a seeded non-terminal gate with `gateType: 'escalation'`, the sweep adopts it per FR-009 and issues zero `superseded` acks against it even when generation would otherwise differ (FR-013 carve-out). | Integration test: seed prior-run `escalation` gate at a drifted generation; assert 0 acks and 1 `openGates` entry with the seeded `gateId`. |
| SC-012 | Adopted `answered` counter initialised to 1 | 100% of `openGates` entries adopted with `status: 'answered'` have `answeredGateSweepCounter[gateId] === 1` at adopt time (FR-010). | Assertion on in-memory `answeredGateSweepCounter` after the adoption pass in the FR-012 test. |
| SC-013 | Per-issue defer on `cockpit_gate_list` error | Given a `cockpit_gate_list` error injected for one in-scope issue, the sweep (a) fires zero `cockpit_gate_open` for that issue this pass, (b) writes a ledger row naming that issue and the error class, (c) completes adoption+drafting for every other in-scope issue, and (d) the run does not abort (FR-014). | Integration test: inject a `cockpit_gate_list` error for one child in a multi-child epic; assert (a)-(d). |
...
- **Answer-document surface for adopted `answered` gates.** No current MCP surface returns the operator's answer document — `cockpit_gate_status` returns `{gateId, status}` and `cockpit_gate_list` returns `{gateId, gateType, generation, status, runId}`. FR-010 preserves an adopted answer only *if* D.12 redelivery fires; otherwise the escape hatch supersedes after 3 sweeps and either re-derives from current labels or re-asks the operator. A cloud-side change that lets the adoption path fetch a prior-run answer document at adopt time (e.g. `cockpit_gate_status` extended to return `answer` on `status: 'answered'`, or a new `cockpit_gate_answer_fetch`) would make FR-010's preservation unconditional. File as a separate issue against generacy-cloud after this repair lands.
...
Clarifications resolved 2026-07-29 via batch 1 (see [clarifications.md](./clarifications.md)):
- Q1=A: broad adoption of every non-terminal row for in-scope issues, with FR-013 drift precedence (FR-009).
- Q2=A: `cockpit_gate_list` fires once per in-scope issue (N+1 calls for an N-child epic), not once per tracking ref (FR-001, SC-008).
- Q3=A: adopted `answered` entries recorded with `answeredGateSweepCounter = 1`; structural limitation documented; follow-up filed (FR-010, Follow-ups).
- Q4=A: generation-drift on adopted gates mirrors the live-path drift branch; `escalation` carve-out preserved (FR-013, SC-010, SC-011).
- Q5=D: per-issue defer on `cockpit_gate_list` error; no new playbook-level retry (FR-014, SC-013, US5).
