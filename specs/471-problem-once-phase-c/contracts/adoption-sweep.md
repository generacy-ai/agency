...
- **Before (4)** — sweep-time `cockpit_gate_open` calls in the § step 3 extended trigger set fire from the § Synthetic-event dispatch block through the D.n rows. The D.n rows' Step 0 pre-draft check reads `cockpit_gate_list` for the same-generation adoption branch on `absent`, and that branch adopts a prior-run row IF the adoption pass has already added it (or IF the issue entered scope after the startup sweep and the adoption pass never saw it). On the drift branch (per [`adoption-drift.md`](./adoption-drift.md)), the adoption pass acks the stale gate `superseded` FIRST, so the sweep's fresh `cockpit_gate_open` at the current-run generation and current-run `runId` produces the sole remaining open for the natural gate (SC-006). The adoption pass is a bulk optimisation that suppresses N+1 list calls at dispatch time; the Step 0 same-generation `absent` sub-branch is the load-bearing correctness site (per [`auto.md § D.1..D.11 Step 0`](../../../packages/claude-plugin-cockpit/commands/auto.md)) that also covers issues joining scope mid-run.
...
3. No ack fires. No fresh `cockpit_gate_open` fires for this natural gate this pass — the row's `gateId` and the current run's derivation for the same natural gate produce DIFFERENT 4-segment `gateId`s (because the two runs' `runId`s differ, per §  gateId idempotency), so the sweep-time `cockpit_gate_open` in step (4) of the ordering above would derive a fresh `gateId` on its own. What actually suppresses the duplicate open is the synthetic-event dispatch pass: every synthetic event this pass produces routes through a D.n row whose Step 0 pre-draft check checks `openGates` first (via the same-generation adoption result). Because this adoption pass has already added the prior-run row to `openGates` under `row.gateId`, the current-run `cockpit_gate_status` at the current-run `gateId` returns `absent`, and Step 0's `absent` sub-branch calls the runId-agnostic `cockpit_gate_list`, finds the same-generation prior-run row (the ONE this adoption pass adopted), takes the same-generation adopt branch, and continues to the next event — the draft-then-open flow does NOT run. The two sites (adoption pass here, Step 0 same-generation absent sub-branch there) are the load-bearing sequence that produces SC-006 ("the sweep issues zero `cockpit_gate_open` for that natural gate"). See [`auto.md § D.1..D.11 Step 0 § `{ status: 'absent' }` sub-branches`](../../../packages/claude-plugin-cockpit/commands/auto.md) for the Step 0 branch text.
