...
Configure automated CI and npm publishing for the agency repo's @generacy-ai scoped packages

**Branch**: `295-set-up-ci-cd` | **Date**: 2026-02-26 | **Status**: Draft
...
7. **Dependency verification**
   - Before publishing, verify `@generacy-ai/latency-*` dependencies are available on npm
   - Agency depends on latency packages being published first

### Publishing details
- All packages published with `--access public`
- `NPM_TOKEN` GitHub secret required (org-level)
- Agency publishes second in the dependency chain (latency → agency → generacy)
