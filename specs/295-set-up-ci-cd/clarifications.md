...
**Answer**: A) `"^0.1.0"` (caret range). Latency is in a different repo — `workspace:*` won't work. The publish workflow's verification step (task 7) already guards against publishing before latency is available. `^0.1.0` is standard for pre-1.0 cross-repo deps and allows compatible patch updates.
...
**Answer**: A) Yes, add to ignore list. The extension has a completely separate publishing pipeline via VSCE (buildout plan issue 1.6) with its own versioning lifecycle tied to VS Code Marketplace pre-release/release channels (odd/even minor versions). Letting changesets manage its version would conflict with the VSCE-oriented versioning scheme.
...
**Answer**: C) Independent versioning. Plugins are optional and may evolve at different rates — no reason to bump a plugin with no changes. The existing `updateInternalDependencies: "patch"` in `.changeset/config.json` already ensures peer dep ranges are updated when `@generacy-ai/agency` is bumped.
...
**Answer**: A) `0.1.0` (minor bump). These packages are pre-stable and APIs will change as external onboarding proceeds. `0.1.0` signals "usable but expect changes" and aligns with latency which is already at `0.1.0`. The first changeset should be a `minor` bump.
...
**Answer**: A) Cancel in-progress on new push, with nuance: For CI (lint/test/build), use `cancel-in-progress: true` — a new push supersedes the old one. For publish workflows, use the same concurrency group but *without* `cancel-in-progress` (queue instead) to avoid aborting a publish mid-way. Specifically: CI: `concurrency: { group: "ci-${{ github.ref }}", cancel-in-progress: true }`, Publish: `concurrency: { group: "publish-${{ github.ref }}" }` (queues, doesn't cancel).
...
**Answer**: A) Yes, enable provenance. One-line addition (`--provenance`), repo is already public (a requirement), and it's increasingly expected for public npm packages. Only requires `id-token: write` permission in the workflow. No reason to defer.
...
**Answer**: A) Add a non-blocking warning. A non-blocking bot comment is low-effort and handles docs/CI-only PRs without requiring a label ceremony. A required check adds friction for trivial PRs; skipping entirely risks missed version bumps in a multi-package monorepo.
...
**Answer**: A) Rely on changesets' built-in idempotency. Changesets handles this natively — `changeset publish` skips versions that already exist on the registry. Re-running the workflow fixes transient failures. Adding retry logic or per-package loops is unnecessary complexity.
...
**Answer**: A) Force-push develop to main. `main` is 111 commits behind `develop` with no valuable main-only history (only initial commits, no merges from develop). A merge would create unnecessary noise. Force-push is the clean one-time reset. Branch protection should be configured *after* this sync.
...
**Answer**: C) Parallel jobs with a summary gate job. Parallel execution for speed, granular failure reporting (immediately see lint vs test vs build failures), and a single "CI Summary" check name for branch protection. The repo already uses Turbo for parallel task execution. Summary gate job is a well-established pattern.
