...
- [ ] T001 [US5] Source the concrete `MIN_GENERACY_VERSION` literal — the first `generacy` release
...
- [ ] T002 [US3] Confirm the D.5 / D.3 post-validate **emission ordering** (research.md § D8) against
...
- [ ] T003 [US5] § step 1 pre-flight — add the version-skew guard (FR-008, Q3). After the
...
- [ ] T004 [US1] § Dispatch table (`auto.md:530–547`) — D.3 action → final-approval gate G.8
...
- [ ] T005 [US1] § Enriched-line dispatch contract E3 (`:463–483`) + E4 checks verdict (`:485–498`).
...
- [ ] T006 [US3] § D.3 (`:724–758`) — keep the **trigger, Source-of-truth, and Step 0** blocks
...
- [ ] T007 [US1] § D.6 (`:818–852`) — remove steps 2–4 (classify → spawn `cockpit-fixer` →
...
- [ ] T008 [US2] § D.13 — new section (`waiting-for:remediation-limit`), modeled on D.4's shape:
...
- [ ] T009 [US1] § Pre-draft check — shared rules → generation-drift branch guard table (`:566–582`).
...
- [ ] T010 [US1] § G.2 (`:1191–1232`) — trigger → D.2 only. Drop the "(artifact and implementation)"
  ...
  FR-002.
- [ ] T011 [US1] § G.4 (`:1258–1353`) — remove subtype **(a)** (validate-red / merge-red) entirely:
...
- [ ] T012 [US3] § G.8 — new gate contract (Implementation-review final-approval): trigger D.3;
...
- [ ] T013 [US2] § G.9 — new gate contract (Remediation-limit) per D.13 (T008): options
  ...
  Contract: `contracts/remediation-limit-gate.md`.
- [ ] T014 [US1] § UI-mode gate mapping table (`:1512–1523`) — remove the **G.4a** row; change the
...
- [ ] T015 [US1] § Generation discriminator table (`:1499–1508`) — `implementation-review` row stays
...
- [ ] T016 [US1] § Ledger action+outcome vocabulary (`:1635–1686`) — D.3 row → final-approval vocab
...
- [ ] T017 [US1] § Invariants (`:1772–1782`) — §1 re-pinned in substance (red → engine gate, not fixer
...
- [ ] T018 [P] [US4] Re-pin the **437** block (`describe` at :2435): the D.6 heading pinned at :2546
...
- [ ] T019 [P] [US4] Re-pin the **449** block (`describe` at :2851): `EXPECTED_GATES` (:892) and
...
- [ ] T020 [P] [US4] Re-pin the **457** block (`describe` at :3213): the D.6 heading pin at :3638
...
- [ ] T021 [P] [US4] Re-pin the **469** block (`describe` at :4666): `469-25` (:5054) enumerates
  ...
  pin at :4678 stays.
- [ ] T022 [P] [US4] Re-pin the **471** block (`describe` at :5243): the
...
- [ ] T023 [P] [US4] Re-pin the **406** block invariants test `406-6` (:1626, `{ n: 1, includes:
...
- [ ] T024 [US4] Add a new `describe("500 slim auto to gates/queue/clarify/merge", …)` block at the
...
- [ ] T025 [US4] **Mandatory playbook re-pin verification** — re-pin
...
- [ ] T026 [US4] Run the pin suite green (SC-001): `pnpm --filter claude-plugin-cockpit test`
