...
**Answer**: Use `Copyright (c) 2026 The Generacy AI Authors`. Use the year of public release (2026), no range. The copyright holder should be "The Generacy AI Authors" (future-proof, covers all contributors, no updates needed if entity name changes). Consistent with the latency repo approach.
...
**Answer**: A) security@generacy.ai. A dedicated security email is professional and separable from individual maintainers. Even if it forwards to one person today, it scales later. Consistent across all Tetrad repos.
...
**Answer**: C) Best-effort. For an early-stage project, overcommitting on SLAs that might be missed is worse than setting realistic expectations. State the team will respond "as soon as possible." Timelines can be tightened later as the team and processes mature. (Latency repo chose "5 business days ack, best-effort fix" — same spirit.)
...
**Answer**: C) General statement. Use: "This project is pre-1.0. Only the latest released version receives security updates. Users are encouraged to keep their installations up to date." Be honest about maturity level — users appreciate transparency.
...
**Answer**: A) Single team confirmed. Use `@generacy-ai/core-team`. Confirm the team exists in the GitHub org first. A team reference is better than individual usernames since membership is maintained in one place. Consistent with latency repo.
...
**Answer**: A) Binary release. Simplest approach with no dependencies. Download the pre-built Linux binary from GitHub releases. Practical for the Linux/WSL2 dev environment.
...
**Answer**: A) GitHub Issue. Attach or paste the summary in the tracking GitHub issue (#298). The audit is a point-in-time artifact tied to the "go public" event. The issue provides the right context and traceability.
...
**Answer**: B) Team notification. History rewrites are destructive. Notify the team before rewriting and give a window to push pending work. Always get explicit approval before force-pushing, even if the team is small. Consistent with latency repo's "maintainer approval required" approach.
...
**Answer**: C) Exclude. It's not a publishable package — omit from CODEOWNERS package-specific mappings. The `*` fallback will still cover it for review assignment. No need to add complexity for a non-standard package.
...
**Answer**: A) Feature branch + PR. Follow the standard workflow: add files on the feature branch, merge via PR to `develop`. Even though the repo isn't public yet, maintaining good workflow hygiene is worth the minimal overhead. The git history audit should run as the final step on the feature branch before merge.
