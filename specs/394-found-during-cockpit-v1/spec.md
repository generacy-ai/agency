...
See `clarifications.md` (Batch 1) for the four questions and the full-prose answers. The load-bearing decisions pinned here:

- **Liveness threshold (Q1)** — 30s per-iteration bounded read on step 4; cross-check fires after N=4 consecutive empty reads (~2 minutes silence). Compound check: silence AND actionable live state (D.1–D.9) in `cockpit status --json`; the status call happens only at the threshold, not on every empty read.
- **"Non-empty line" (Q2)** — trim whitespace, then non-empty. Malformed/truncated JSON is consumed as an event; whitespace-only lines are dropped as line-framing hygiene. Content-shape heuristics (must-start-with-`{`, etc.) are prohibited.
- **Regression location (Q3)** — fixtures under `packages/claude-plugin-cockpit/tests/fixtures/`; assertions added to the existing playbook-verification suite where the #388/#390 checks live (exact suite file named in `plan.md`). Executable verification only, no standalone `.md` regression file.
- **Detection vs recovery (Q4)** — FR-005's "no new recovery machinery" constrains the recovery path only (re-arm + step 3). The 30s bounded read is the sole new detection mechanism admitted, and it is a hard requirement of FR-004 (a dead reader cannot event-drive its own diagnosis).
...
- [ ] A concrete N value is pinned in the prose: step 4's read is per-iteration bounded to **30 seconds**, and the cross-check fires after **N=4 consecutive empty reads** (~2 minutes of silence). The 30s bound mirrors the watcher's own poll cadence; the ~2-minute threshold is far above normal event latency and recovery is idempotent so false-positive cost is nil.
- [ ] The cross-check is **compound** and stated verbatim: silence alone is normal during long implement stretches; the cross-check fires only on the conjunction of (a) N=4 consecutive empty reads AND (b) `cockpit status --json` reports at least one issue in a D.1–D.9 actionable transition class. The `cockpit status --json` call happens **only at the threshold** (not on every empty read).
...
- [ ] Fixtures live under `packages/claude-plugin-cockpit/tests/fixtures/`. Behavioral assertions are added to the **existing** playbook-verification suite where the #388/#390 checks already live (the exact suite file is named in `plan.md` rather than guessed here — the #388/#390 fixes shipped with static + behavioral verification, so the home exists). Only if it is genuinely absent, create a new suite file alongside the plugin's existing test layout. A standalone `.md` regression file is not acceptable — this must be executable verification.

## Functional Requirements

| ID | Requirement | Priority | Notes |
|----|-------------|----------|-------|
| FR-001 | `packages/claude-plugin-cockpit/commands/auto.md` step 4 (Main loop) prose is amended to state, verbatim, that new lines from the background watch process output are read **unfiltered** and that every non-empty line is an event. **"Non-empty"** is defined operationally at the read boundary: **trim leading/trailing whitespace, then treat any remaining non-empty content as an event** — including malformed or truncated JSON. Whitespace-only lines are skipped as line-framing hygiene (they carry no information); an unparseable line still signals "something happened" and triggers the step 4(a) live-state re-check, which absorbs it safely. Content-shape heuristics (e.g. "must start with `{`") are prohibited: a truncated flush would be dropped silently, which is under-delivery — the failure class this fix exists to kill. The amendment carries a one-sentence rationale inline: the stream carries more than one event shape (legacy per-issue envelopes without `type`; S8 synthetic aggregates with `type`); over-delivery is absorbed by the live-state re-check (4a); under-delivery is silent loop death. | P1 | The rule is pinned inside step 4 (not in an appendix, not in the invariants section) because that is where the session was under-instructed at T-S4. |
...
| FR-004 | `auto.md` step 4 (Main loop) gains a per-iteration bounded read: each read from the background watch process output is bounded to **30 seconds**. `auto.md` step 5 (Watch re-arm) gains a "Liveness cross-check" sub-step: if the watch process is alive AND **N=4 consecutive empty reads** have elapsed (~2 minutes of silence) AND `cockpit status --json` shows at least one issue in a D.1–D.9 transition class, re-arm the stream reader and re-run step 3 (startup sweep). The 30s bound and N=4 threshold are pinned verbatim in the prose. The `cockpit status --json` call runs **only at the threshold**, not on every empty read. | P1 | Defense-in-depth: even if a future prompt-side amendment fails to prevent a filter, the loop catches the silent outage itself. The 30s bounded read is the **only new detection mechanism admitted** by this fix — a dead reader cannot event-drive its own diagnosis, so some clock is a hard requirement. Recovery reuses shipped idempotent paths (L.5). |
| FR-005 | The liveness cross-check's **recovery path** is exactly: re-arm the reader (same mechanism as watch-death re-spawn) + re-run step 3. No new recovery machinery is introduced — this constraint applies to the **recovery path only**, not to the detection path (the 30s bounded read + empty-read counter from FR-004 is admitted as detection machinery). The prose states, verbatim, that both recovery steps are idempotent per the L.5 rule so no duplicate action can result. | P1 | Every ledger consequence of the recovery is already handled — startup-sweep events produce their own ledger lines per the mandatory-per-dispatch rule. |
...
| FR-007 | A playbook behavioral regression check is defined per the S6/S9 verification pattern, feeding a fixture stream with **both** event shapes (legacy per-issue envelope + S8 synthetic aggregate) and asserting both reach the dispatch table. A second fixture asserts the liveness cross-check triggers a startup-sweep re-run on the (alive-process, zero-events, actionable-live-state) condition. **Fixtures live under `packages/claude-plugin-cockpit/tests/fixtures/`**; assertions are added to the **existing** playbook-verification suite where the #388/#390 checks live (exact suite file named in `plan.md`). A new suite is created only if the existing home is genuinely absent. Executable verification only — no standalone `.md` regression file. | P1 | Prevents a future edit from silently re-introducing the filter or removing the cross-check. |
...
- The playbook execution environment supports a per-iteration bounded read on the watch stream with a 30-second timeout — this is the sole new mechanism admitted by this fix (see FR-004). The threshold values are pinned in prose only: 30s bounded read + N=4 consecutive empty reads (~2 min silence); no new configuration surface is introduced.
- "Non-empty line" at the read boundary means: trim whitespace, then non-empty. Whitespace-only lines are dropped as line-framing hygiene; malformed/truncated JSON is consumed as an event so the step 4(a) live-state re-check catches whatever state changed.
