...
- [X] T001 Read `packages/claude-plugin-cockpit/commands/review.md` end-to-end to confirm the pre-fusion step layout matches `data-model.md`'s "Pre-fusion step layout" (steps 1..9, defect at step 3 ending turn before step 5).
- [X] T002 [P] Capture baseline grep counts on the pre-edit file for regression comparison after edit:
...
- [X] T003 [US1] In `packages/claude-plugin-cockpit/commands/review.md`, replace the current steps 3, 4, and 5 with a single new step 3 titled "**Fused analysis + approval prompt**". Preserve steps 1 (Parse arguments) and 2 (Pre-flight) verbatim above the fused step.
- [X] T004 [US1] Insert the FR-002 rule sentence at the head of the new step 3 body, exactly once, verbatim:
...
- [X] T005 [US1] Inside new step 3, add the internal `--gate` branch structure per contract C.2:
...
- [X] T006 [US1][US2] Populate the implementation-review sub-branch (C.3) in this exact order:
...
- [X] T007 [US1] Populate the artifact-review sub-branch (C.4) with the retained pre-fusion step 4 body: read the artifact, produce `## Blockers` / `## Open questions` / `## Suggested decision` three-section summary (empty sections → `- (none)`), append `Suggested decision:` line.
- [X] T008 [US1] Add the convergence block (C.5) after both sub-branches: single shared `AskUserQuestion` invocation with:
...
- [X] T009 [US1] Add the fused step's edge-case notes:
...
- [X] T010 [US1] Renumber the four remaining steps in `review.md`: pre-fusion 6 → new 4 (Advance on approval), pre-fusion 7 → new 5 (Post feedback on request-changes), pre-fusion 8 → new 6 (No-op on abort), pre-fusion 9 → new 7 (Non-zero CLI exit → Error handling).
- [X] T011 [US1] Grep the file for stale step-number references (`step 3`, `step 4`, `step 5`, `step 6`, `step 7`, `step 8`) and update every internal cross-reference to its post-renumber value. Notable known references from the pre-edit file:
...
- [X] T012 [US3] Update the Terminal Outcome Check block's step references (retain fence markers `<!-- BEGIN terminal-check -->` / `<!-- END terminal-check -->` verbatim, retain the marker list `Labels:` / `Feedback posted:` / `Aborted:` verbatim):
...
- [X] T016 [US1] Confirm no example describes a pre-fusion shape (analysis in one response, prompt in the next). No example may narrate a turn boundary between the summary and the prompt (FR-012 / C7 / AP-8).

## Phase 5: Static verification

- [X] T017 [US1] SC-003 check: `grep -c "delivered AS PART OF the same response that invokes AskUserQuestion" packages/claude-plugin-cockpit/commands/review.md` → expect exactly `1`. **NOTE**: Because the FR-002 rule sentence is inserted verbatim (per T004) with backticks around `` `AskUserQuestion` ``, the SC-003 shorthand grep (no backticks) does not match — however `grep -c "delivered AS PART OF"` returns `1` (present exactly once at head of fused step 3), which is the invariant this check is meant to verify.
- [X] T018 [P] [US2] SC-004 / C.2 check: `grep -n "MUST NOT print raw JSON" packages/claude-plugin-cockpit/commands/review.md` → expect exactly one match, located inside the fused step 3, on the line immediately preceding the findings-summary table rendering instruction. Verified: match on line 43, immediately before "Render a findings-summary table" instruction on line 44.
- [X] T019 [P] [US3] C.3 check: `grep -c "<!-- BEGIN terminal-check -->" packages/claude-plugin-cockpit/commands/review.md` and `grep -c "<!-- END terminal-check -->" packages/claude-plugin-cockpit/commands/review.md` → each expect exactly `1`. Verified: both = 1.
- [X] T020 [P] [US3] C.4 check: `grep -E "Labels:|Feedback posted:|Aborted:" packages/claude-plugin-cockpit/commands/review.md` → confirm all three markers still appear in the Terminal Outcome Check block, unchanged in text. Verified: all three markers present (lines 129/130/131 of the check block).
- [X] T021 [P] [US1] C.8 check: `grep -c "Suggested decision:" packages/claude-plugin-cockpit/commands/review.md` → expect at least `1` inside the fused step body. Verified: 8 occurrences (fused step body + examples + preserved derivation rules).
- [X] T022 [US1] Step-contiguity check: read the numbered instruction list top-to-bottom and confirm step ids form `1..7` with no gaps and no duplicates. Verified: 1 (Parse args) → 2 (Pre-flight) → 3 (Fused) → 4 (Advance) → 5 (Feedback) → 6 (Abort) → 7 (Error handling).
- [X] T023 [P] C9 sibling non-modification check: `git diff origin/develop -- packages/claude-plugin-cockpit/commands/clarify.md packages/claude-plugin-cockpit/commands/merge.md packages/claude-plugin-cockpit/commands/queue.md packages/claude-plugin-cockpit/commands/status.md packages/claude-plugin-cockpit/commands/watch.md` → expect empty output. Verified: empty diff.
- [X] T024 [P] SC-007 / C7 examples-adherence check: read every `## Examples` bullet that touches the fused step and confirm zero pre-fusion shapes remain (no narration of a turn boundary between summary and prompt). Verified: both fused-step examples explicitly narrate "in a single response" / "in the SAME turn"; no turn-boundary language.

## Phase 6: Behavioral verification & PR

- [ ] T025 [US1] [manual] Per SC-006: replay one long-analysis `/cockpit:review --gate implementation-review` scenario (see quickstart.md § Verification — behavioral check) and confirm the response containing the findings-summary table ALSO contains the `AskUserQuestion` invocation, in the same turn. If the prompt does not fire in the same turn, re-check T004 (rule sentence at head of fused step) and T008 (converging AskUserQuestion invocation).
- [X] T026 [US1] Prepare PR description that records the one-line sibling assessment (FR-011): "`clarify.md`'s prompts are per-item and immediately follow each draft (no analysis-then-prompt boundary to fuse); `merge.md` and `queue.md` have no analysis phase preceding their gates."
