...
| FR-010 | The existing Terminal Outcome Check is retained as a secondary backstop covering steps 6-8 (post-renumber: 5-7). | P2 | Primary guarantee moves from "remember at the end" (positional) to "the deliverable IS the prompt" (structural, via FR-001+FR-003). |
...
- Changes to the four artifact-review sub-branches beyond bringing them under the single fused step's rule sentence.
