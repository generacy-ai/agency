...
**Answer**: C) Hybrid — start with a live epic and, if any gate type doesn't fire naturally within the run window, seed a synthetic follow-up to cover the missing types.

**Rationale**: A live epic preserves the "drive a real epic" fidelity the epic asks for, but gate types like escalation, supersession, and offline rarely fire naturally — seeding synthetic follow-ups closes those coverage gaps deterministically. Pure-live risks never exercising some gates; fully-synthetic sacrifices realism.
...
**Answer**: A) Coverage-complete — run ends when every gate type in FR-003–FR-007 has been exercised at least once (including approve + request-changes + free-text round + supersession + offline); epic may still be in-flight, noted in the report.

**Rationale**: The deliverable is a report proving each remote gate type works end-to-end, so a stopping rule keyed to gate-type coverage matches the acceptance criterion and pairs cleanly with the Q1 hybrid. Epic-terminal forces artificial gates and extra churn for no added evidence; operator-judged is too vague to reproduce.
...
**Answer**: B) Red-check merge — cause a merge to hit red checks that the bounded fixer subagent cannot resolve, so the driver escalates to a human gate.

**Rationale**: This is the canonical, deterministically-reproducible escalation (D.6 / G.4(a)): a genuinely unfixable red check makes the bounded fixer run once, stay red, and fire the escalation gate. Repeated request-changes drives the review-verdict gate (G.2), not an escalation. Before the run, confirm the UI presenter renders the escalation gateType and file a gap if not.
...
**Answer**: B) Advance via a separate action — while a gate is open, cause the underlying phase to advance by another route (CLI cockpit_advance, another gate answer, or a GitHub label change), then submit the inbox answer against the old generation.

**Rationale**: This is exactly the plan's "Stale answers" scenario (state moves on before the UI answer arrives; the session validates against live state and acks superseded), and advancing via a separate action makes it controlled and reproducible. The inline-answer and driver-retry-race options are timing-dependent and flaky.
...
**Answer**: A) Network-level — block outbound connectivity from the cluster to the generacy.ai inbox transport (firewall rule / unplug) while the process keeps running; restore connectivity to observe delivery.

**Rationale**: FR-011's documented behavior is "answer stored as answered, delivered on reconnect" via the cluster relay handshake, so severing the authenticated relay WS and restoring it exercises precisely that redelivery-on-reconnect path. Stopping the driving process conflates offline delivery with the separate session-restart / re-derive path. Confirm the relay endpoint to block and file a gap if redelivery diverges from the plan.
