...
| FR-003 | Confirmation `AskUserQuestion` uses the question string ``Run `generacy cockpit queue <epic-ref> <phase>`?`` with both tokens interpolated. Options remain `Confirm` / `Cancel` in that order; `multiSelect: false`; header stays `Queue phase`. | P1 | |
...
| FR-005 | On Confirm, the plugin runs `generacy cockpit queue <epic-ref> <phase>` from the repository root via the Bash tool, capturing stdout, stderr, and exit code separately. | P1 | Pre-flight `command -v generacy` remains; MISSING_BINARY class applies unchanged. |
...
| FR-008 | The double-prompt policy is decided and documented in the command file: either (a) pass `--yes` to the CLI so the plugin's `AskUserQuestion` is the sole gate, or (b) drop `--yes` and rely on the CLI's own confirm. Exactly one of these is documented; the invocation in step 4 matches. | P1 | Needs clarification — see Assumptions. |
...
- `--yes` handling: **[NEEDS CLARIFICATION]** — pick one of (a) plugin passes `--yes`, CLI skips its own confirm, or (b) plugin does not pass `--yes`, CLI runs its own confirm. Only one gate should reach the user.
