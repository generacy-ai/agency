# Clarifications for Feature 374: publish claude-plugin-cockpit as @generacy-ai/claude-plugin-cockpit
...
**Answer**: A — version "0.0.0" + minor bump → first publish 0.1.0. The plugin is new and pre-stable; matching spec-kit's 1.0.x line (D) would claim a stability we haven't earned, and the smoke test (generacy-ai/tetrad-development#88) is still ahead.
...
**Answer**: B — no agency block. Cockpit is a Claude-side plugin only; its identity lives in `.claude-plugin/plugin.json`, and the delivery path (`generacy setup build` copying `commands/` from `node_modules`) never reads agency metadata. An empty agency block would imply Agency-runtime discovery that doesn't exist — dead surface.
...
**Answer**: A — omit `type`/`main`/`module`/`types`/`exports`/`bin`/`scripts` entirely. The package ships static markdown; `publish-preview`'s `pnpm -r run --if-present build` skips script-less packages cleanly. Not C: placeholder fields are dead surface.
...
**Answer**: A — yes, update README in this feature: add a short "Distribution" section describing the npm path (cluster setup installs it; `generacy setup build` wires commands) while KEEPING the marketplace instructions for standalone/non-cluster users — npm doesn't replace the marketplace, it adds the zero-step cluster rail. Amends FR-006's isolation to include `README.md`; `commands/*.md` stay untouched.
...
**Answer**: A for author/license/repository (mirror sibling, `directory` subpath), with an amended description: "Claude Code plugin providing /cockpit:* commands for running Generacy speckit epics (watch, status, queue, clarify, review, merge)" — "Tetrad workflows" undersells that it works for any team running a cluster. Keywords as proposed.
