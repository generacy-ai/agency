...
- The skill then creates a tracking issue seeded with the resolved refs (reuse the Form-3 `--new` path, minus the title prompt):
  - Title convention: `Tracking: auto session YYYY-MM-DD — #223 #224 #226` (truncate the ref list sensibly for long lists).
...
- [ ] The tracking issue carries the `cockpit:tracking` label and the title convention above.
