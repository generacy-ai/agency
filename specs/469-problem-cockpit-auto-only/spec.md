...
| FR-014 | `runId` MUST be derived exactly once, at pre-flight, at the same point the ledger filename is computed. Every downstream consumer (parent loop, subagent dispatches, gate verbs) receives the pre-computed value as an explicit literal — no consumer re-derives it, even by the same rule. | P1 | A's (Q7) failure mode is silent: two independent derivations that agree today can diverge under a filename-format change or a stale-ledger race. Compute-once eliminates that class of failure by construction. |
| FR-015 | Subagents that issue any gate verb MUST receive the current run's `runId` as an explicit literal in the dispatch prompt. Subagents MUST NOT re-derive `runId` from the ledger filename or any other source. | P1 | Matches how every other run-scoped value (epic ref, gateId, cursor, prompts) already travels in `auto.md`. B (subagent re-derives) requires a one-file-in-the-directory invariant that does not hold; C (env var / shared file) adds a global surface for one value that has a clean propagation path. |
| FR-016 | Because `runId` is *optional* on every MCP gate schema, a dispatch path that forgets to pass it silently derives a 3-segment key and mismatches everything else in the run. The spec MUST enumerate every dispatch path that can issue a gate verb — the six Step-0 blocks (D.1, D.2, D.3, D.4, D.7, D.11), the startup sweep's `cockpit_gate_open` calls, and D.12's `cockpit_gate_ack` — and an automated test MUST assert each path carries the run's `runId`. Sampling one call site is insufficient. | P1 | Same class of silent-degradation failure as the write/read split in FR-009, one layer down; will not surface as a crash. The enumerating test is the only mechanism that catches a newly added dispatch path that omits `runId`. |
...
| SC-010 | Single-derivation invariant | `runId` is computed exactly once per run, at pre-flight, and every downstream consumer receives that literal. | Code inspection / static analysis: exactly one call site derives `runId`; all other uses reference the derived value. Test: mutate the derivation site's output and assert every gate verb sees the mutation. |
| SC-011 | Dispatch-path enumeration coverage | An automated test enumerates every gate-verb-issuing dispatch path (the six Step-0 blocks D.1/D.2/D.3/D.4/D.7/D.11, the startup sweep's `cockpit_gate_open` calls, D.12's `cockpit_gate_ack`) and asserts each carries the run's `runId`. | Test suite listing dispatch paths by name; failure if a new path is added without a corresponding assertion. |
...
### Behaviour change introduced by this phase — re-invocation is a new run

Before this phase, `gateId = hash(issueRef, gateType, generation)` had no run component, so re-invoking `/cockpit:auto` against the same tracking ref resumed prior gate identity by construction — a second invocation derived the same key, the Step-0 check found the still-open gate, and the reuse branch fired. This is what `auto.md:283` means by "sweep-derived and live-derived `gateId`s coalesce".

After this phase, it does not. A re-invocation is definitionally a new run: pre-flight mints a new ledger file (`auto.md:209`), which by FR-001 mints a new `runId`. Trace an interrupted-then-re-invoked run (context exhaustion, `Ctrl-C`, cluster restart — all routine):
...
That is the same duplicate-inbox-gate symptom this epic exists to remove, arriving through a door the epic opened. It is stated here so an operator who hits this finds it written down rather than reverse-engineering it from two identical gates.

The clean fix — startup sweep adopts pre-existing non-terminal gates for the tracking ref into `openGates` before drafting anything, using `cockpit_gate_list({issueRef, gateType: <omitted>})` which is runId-agnostic and whose rows now carry `runId` as a first-class field (generacy-cloud#892) — is filed as a follow-up and is deliberately NOT in this PR's scope (Phase C already carries more than its original scope). It is a second, independent reason `cockpit_gate_list` must stay runId-agnostic (reinforces FR-011): if a future change ever "fixes" list to filter by `runId` by default, it would foreclose this repair before it is built. generacy-cloud#894 should stay opt-in filtering for that reason.
...
- Adopting pre-existing non-terminal gates for a tracking ref into `openGates` on session startup (the follow-up that repairs the "re-invocation is a new run" behaviour change noted in Assumptions). Filed separately; deliberately NOT in this PR to keep the diff bounded.
- Session-resume semantics for `/cockpit:auto` itself. `cockpit_resume` is a per-issue engine action (`auto.md:829`) that clears `agent:error` / `failed:*` labels; it is not a session-level restore surface. `/cockpit:auto` does not currently expose a session-resume surface, and this issue does not add one. A re-invocation is definitionally a new run (see the Assumptions subsection above). If a future session-resume surface is added, it needs its own clarification.
- Rejected: subagents re-deriving `runId` from the ledger filename they discover on their own (see FR-015 — requires a one-file-per-directory invariant that does not hold and would drift under any filename-format change).
- Rejected: passing `runId` via environment variable or shared file (see FR-015 — adds a global surface for one value that has a clean explicit-literal propagation path).
