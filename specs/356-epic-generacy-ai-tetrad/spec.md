...
**Branch**: `356-epic-generacy-ai-tetrad` | **Date**: 2026-06-29 | **Status**: Draft
...
| FR-002 | The command MUST accept a single positional `<epic-ref>` argument, either bare (`356`) or fully-qualified (`owner/repo#356`). Argument parsing follows the same pattern as `/cockpit:clarify` and `/cockpit:watch`.                                       | P1       | Consistency across cockpit verbs. |
...
| FR-004 | The output file path MUST be derived from the epic ref and title: `docs/epic-<kebab-slug>-plan.md`. The slug is generated from the epic issue title, lowercased, with non-alphanumerics collapsed to `-`.                                                 | P1       | Deterministic so reruns idempotent. |
| FR-005 | If the file exists, the command MUST NOT overwrite. It MAY append missing canonical sections (US2) but only after explicit developer confirmation, with an `<!-- generacy-cockpit:appended -->` marker preceding the appended block.                      | P1       | Human-led — no destructive writes. |
...
| FR-010 | The skeleton MUST embed the epic ref (e.g. `**Epic**: generacy-ai/tetrad-development#85`) and any `Phase: …` / `Tier: …` metadata extractable from the epic body as a front-matter block at the top of the file.                                          | P2       | Aids cross-linking. |
...
- Epic issues follow the standard body format used elsewhere in this epic (Phase / Tier / Owns / Acceptance / Depends on lines), so the metadata extraction in FR-010 has something stable to parse.
