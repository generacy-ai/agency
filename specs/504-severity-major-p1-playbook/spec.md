...
|----|-------------|----------|-------|
| FR-001 | D.5 MUST add a `waiting-for:implementation-review`-absent guard: when that label co-occurs with `completed:validate`, D.5 does not merge and defers to D.3 / G.8. | P0 | Core safety fix; `auto.md:813-831` |
| FR-002 | The deferral MUST be observable in the ledger (a D.5 row indicating "deferred to G.8 / implementation-review pending" rather than a merge outcome). | P1 | Ledger line at `auto.md:826` |
| FR-003 | D.13 and G.9 MUST specify the findings retrieval procedure: latest linked-issue comment whose body starts with `## Remediation limit reached`. | P1 | `auto.md:1048`, `:1506-1512` |
| FR-004 | D.13 and G.9 MUST document the bullet format contract `- <file>:<line> — <title>` and an explicit empty/`(none)` fallback when no matching comment exists. | P1 | Mirrors engine `phase-loop.ts:1411-1421` |
| FR-005 | G.8 prose MUST stop asserting a per-implementation-review gate-body findings artifact that the on-ci-green branch never writes; keep the `(none)` fallback. | P2 | `auto.md:1478`, `:1480` |
