...
- [X] T001 Re-read the two contracts before editing so the prose matches them verbatim:
...
- [X] T010 [US1] Add the `waiting-for:implementation-review`-absent guard to D.5 Dispatch in
...
- [X] T011 [US1] Extend the D.5 ledger outcome enum in the same D.5 block with the token
...
- [X] T020 [US2] Rewrite D.13 step 1 in `packages/claude-plugin-cockpit/commands/auto.md`
...
- [X] T021 [US2] Mirror the same fetch/selection/bullet/`(none)` contract into the G.9 Presentation
...
- [X] T030 [P] [US3] G.8 prose (§ `G.8 — Implementation-review final-approval gate`,
...
- [X] T031 [P] [US3] `packages/claude-plugin-cockpit/lib/gate-status-check.ts` docstring `:164-165`:
...
- [X] T032 [P] [US3] `cockpit.auto.agents` role-selector list in `auto.md` (~`:262`): remove the
  `fixer` key. Resulting keys: `default` / `clarifier` / `reviewer` / `validator` / `diagnoser`.
  (FR-007; data-model E5)
- [X] T033 [P] [US3] Fix the stale comment in
...
- [X] T040 [US1] [US2] [US3] Re-pin `packages/claude-plugin-cockpit/tests/playbook-verification.test.ts`
  ...
  (FR-009; research R6)
- [X] T041 Run the suite green: `pnpm --filter @generacy/claude-plugin-cockpit test`
