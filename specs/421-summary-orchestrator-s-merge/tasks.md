...
- [ ] T013 [US1] Walk each of the five `quickstart.md` verification scenarios (S1 regression, S2 new path, S3 dedup, S4 post-advance re-gate, S5 other `blocked:*` → D.10) against the edited prose; document any mismatch and re-open the corresponding phase task.
- [ ] T014 [US1] Run `git diff packages/claude-plugin-cockpit/commands/auto.md` and confirm every one of the ten edit sites in `plan.md` § Edit sites appears in the diff; nothing else outside those sites should change.
