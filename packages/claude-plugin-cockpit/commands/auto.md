...
6. **Remove from openGates**: on `applied` / `superseded` / `failed`, `openGates.delete(event.gateId)`. A revised-draft re-open (step 5 handler-ambiguity path) creates a NEW record under a fresh `gateId` — the recomputed generation discriminator changes the `gateKey`, so the derived `gateId` changes; the prior record is **marked `superseded`** and retained in `openGates`, so a late answer arriving under the OLD `gateId` is acked `superseded` by gateId identity (step 2 — the frozen down-path answer carries no `generation`).
