...
| D.3 | `waiting-for:implementation-review` | Final-approval gate G.8 (findings from gate body, **no subagent**) → `approve` → post-validate: cockpit merge path / legacy: `cockpit_advance(issue, gate="implementation-review")` / undetectable: fail closed; `hold`/`reject` → no-op (label stays, gate re-fires) |
...
**Dispatch**:
1. **Resolve `checks` verdict.** Prefer the enriched line's `checks` field (E4). `checks: "green"` → merge (step 2). `checks: "red"` → fall through to D.6. `checks` **absent OR `pending`** → single authoritative `cockpit_status(issue=<issue-ref>, json=true)`: verify `checks_state == "green"` and no infrastructure/runner failures; a red fallback verdict falls through to D.6.
2. **Merge**: `cockpit_merge(issue=<issue-ref>)` (squash, branch delete per the tool's default; the tool resolves the issue's linked PR internally — never pass a PR ref directly).
3. **No gate.** Operator judgment was recorded at `waiting-for:implementation-review` (D.3); `validate` + green checks is mechanical.
...
**Trigger**: The re-check step reads a live state whose transition class is not one of D.1–D.9 (including D.9a/b/c), D.11, or D.13: (a) S8 adds a new transition class the playbook doesn't know, (b) the streamed event conflicts with the live state and neither is dispatchable, (c) `cockpit status --json` returns an unexpected shape, **(d) any state token (`waiting-for:*` OR `blocked:*`) does not match a Trigger in any § Dispatch row (D.1–D.9c, D.11, or D.13)** — future `blocked:*` labels without their own dispatch row land here, not in D.11. `waiting-for:remediation-limit` matches D.13 and MUST NOT fall through to D.10.
...
| `remediation-limit` | PR head SHA + remediation counter |
