...
1. **Parse arguments + pre-flight.** Recognize exactly one of four invocation forms per invocation — the tracking ref is the run's identity under all four forms:
   ...
   **Then**, optionally echo the engine version for operator context (**advisory only** — FR-001, Q1=D). Run `generacy --version` (`generacy` exposes `.version(VERSION)` — the same CLI `auto` already invokes; no new MCP field) and echo the emitted string in the startup context. This echo is **informational only**: it MUST NOT exit non-zero on the version value and MUST NOT gate the run. Version cannot distinguish a compatible engine from an incompatible one — npm stable `0.10.2` is pre-#1120, while the builds that actually ship epic generacy-ai/generacy#1120's post-validate `implementation-review` gate move report versions *below* it (preview `0.0.0-preview-*`, source `0.1.1`), and a stock #1120-bearing engine with `reviewPhaseEnabled` / `ciMergeGateEnabled` off still fires the gate pre-validate. The **authoritative** compatibility signal is therefore the runtime gate-placement observation at § D.3 (does `waiting-for:implementation-review` co-occur with `completed:validate`?), not this echo. No engine surface reports the gate model at pre-flight (`generacy cockpit --help` exposes only watch/doorbell/status/advance/context/merge/queue/resume/scope/mcp), so a missing or unparseable `generacy --version` here is **not** fatal — the run proceeds and defers to the runtime signal.
   ...
   New-engine + old-`auto` skew remains inert by construction: an old `auto` lacks the D.13 / D.14 / G.8 / G.9 / G.10 rows, so the engine's new gates fall through to the D.10 unknown-state escalation — a visible escalation, not a silent strand — so no guard is needed for that direction.
...
3. **Startup sweep.**
   ...
   **UI-mode extended trigger set (Q2=B)**. When `ResolvedGateMode === "ui"`, the startup sweep re-opens remote gates via `cockpit_gate_open` for **every persistent gate-trigger state** — a superset of the `waiting-for:*` baseline. Under `local`, the sweep behaves EXACTLY as today. Trigger states:
   ...
   - **All `waiting-for:*` labels**: `waiting-for:clarification` (D.1), `waiting-for:<artifact>-review` (D.2 — spec / clarification / plan / tasks), `waiting-for:implementation-review` (D.3), `waiting-for:manual-validation` (D.4), `waiting-for:merge-conflicts` (D.11 — co-occurs with `blocked:stuck-merge-conflicts`), `waiting-for:remediation-limit` (D.13 — G.9), `waiting-for:ci` (D.14 — G.10; co-occurs with `agent:paused`). D.13 and D.14 are persistent engine pauses that do NOT self-re-fire: omitting them here means a `--gates=ui` restart never re-opens the remote gate and the issue parks forever.
...
| D.14 | `waiting-for:ci` | **enriched line** | fallback |
...
Label-driven classes (D.1–D.11, D.13, D.14) plus the UI-mode gate-answer completion class D.12 (fires only under `ResolvedGateMode === "ui"`). Authoritative state resolves per step 4a / § Enriched-line dispatch contract: enriched `to` / `labels` (and, for D.5/D.6, `checks`) are the source of truth for label-driven classes; the per-event `cockpit_status(epic=<epic-ref>, json=true)` re-check is retained for D.8, D.10, D.11 and fires as fallback for bare lines or absent/`pending` `checks`. Ledger-only rows (D.9, D.9a, D.9b, D.9c, D.9d) skip any query entirely per § Invariants #8's cost contract. **D.12**: under `ResolvedGateMode === "ui"`, every gate contract G.1–G.10 that maps to the wire record (per § UI-mode gate mapping) OPENS via `cockpit_gate_open` instead of `AskUserQuestion`; the operator's answer arrives as a D.12 `gate-answer` event routed onto the SAME downstream handling the local `AskUserQuestion` path performs today, closing the record with `cockpit_gate_ack(applied | superseded | failed)`. Each dispatch is **CLI verb + optional subagent + optional gate**; no dispatch invokes a `/cockpit:*` slash command (invariant §4).
...
| D.13 | `waiting-for:remediation-limit` | Remediation-limit gate G.9 (findings from the `<!-- generacy-remediation-limit -->` / `## Remediation limit reached` issue comment, **no subagent**) → `resume remediation` → `cockpit_advance(issue, gate="remediation-limit")`; `stop` → exit clean, no label writes |
| D.14 | `waiting-for:ci` | CI merge-readiness gate G.10 (verdict from the `<!-- generacy-ci-pause -->` / `## CI merge readiness paused` issue comment, **no subagent**) → `resume (re-check CI)` → `cockpit_advance(issue, gate="ci")` (engine re-arms at `validate`); `stop` → exit clean, no label writes |
...
Rules every § Dispatch **Step 0 — pre-draft gate-status check** obeys, stated once so the eight Step 0 blocks (D.1, D.2, D.3, D.4, D.7, D.11, D.13, D.14) cannot drift apart. All are dead prose under `ResolvedGateMode === "local"` (Step 0 is skipped entirely there).

**`runId` (fourth input under `runIdEnabled === true`) — per #469 / FR-010 / FR-014.** The pre-draft check's `gateId` uses FOUR inputs when `runIdEnabled === true`; the fourth is the pre-flight-derived `runId` (per § step 1 Pre-flight `runId` derivation and § In-memory loop state additions above). The `runId` is threaded on the `cockpit_gate_status` payload as an explicit literal read verbatim from loop state — NEVER re-derived by the Step 0 site, even by the same rule (per V2 / FR-014). Under `runIdEnabled === false` the field is OMITTED from the wire payload (V6) and the pre-#469 3-input identity applies. The eight Step 0 blocks name the same four inputs by construction, so sweep-derived and live-derived `gateId`s coalesce when content is unchanged AND the run is the same. The compute-once + explicit-literal invariant extends to subagent dispatch prompts — see § UI-mode gate mapping header note "Subagent dispatch prompt template addition".
...
**Recoverability per `gateType`.** Six of the frozen enum values map 1:1 onto a single Step-0 row, so condition 2 is satisfied by condition 1 alone. `escalation` does not:
...
**Trigger**: The re-check step reads a live state whose transition class is not one of D.1–D.9 (including D.9a/b/c), D.11, D.13, or D.14: (a) S8 adds a new transition class the playbook doesn't know, (b) the streamed event conflicts with the live state and neither is dispatchable, (c) `cockpit status --json` returns an unexpected shape, **(d) any state token (`waiting-for:*` OR `blocked:*`) does not match a Trigger in any § Dispatch row (D.1–D.9c, D.11, D.13, or D.14)** — future `blocked:*` labels without their own dispatch row land here, not in D.11. `waiting-for:remediation-limit` matches D.13 and `waiting-for:ci` matches D.14; neither MUST fall through to D.10.

**Any `waiting-for:*` OR `blocked:*` label without a matching dispatch row IS an unrecognized state.** "Known but not actionable" is not a permissible classification outcome — the § Dispatch table is the exhaustive list of `waiting-for:*` and `blocked:*` states the loop may treat as no-ops (D.9, D.9a, D.9b, D.9c) or dispatch to a dedicated gate (D.11, D.13, D.14). If the table does not name it, D.10 fires — verbatim state in the presentation block.
...
1. **Fetch and render the remaining findings from the engine's issue comment.** The engine writes the non-converged findings as a plain GitHub issue comment on the linked issue (`phase-loop.ts` `REMEDIATION_LIMIT_MARKER` body builder (on-remediation-limit gate)); the gate record carries no findings (`cockpit_gate_status` returns `{gateId, status}` only), so the comment is the sole source in BOTH local and UI gate modes. The engine prepends the hidden marker `<!-- generacy-remediation-limit -->` as the FIRST line of every remediation-limit comment (`phase-loop.ts:56`; it dedups re-posts on it) and the `## Remediation limit reached` heading is only the SECOND line — so a `startsWith` test against the heading NEVER matches and MUST NOT be used. Retrieval: `gh issue view <issue-ref> --json comments` → filter to comments whose `body` `startsWith` the exact, case-sensitive marker `<!-- generacy-remediation-limit -->` → **pre-marker engine fallback**: if no comment starts with the marker, filter instead to comments whose `body` `includes` the exact, case-sensitive string `## Remediation limit reached` → if neither filter matches, render the `(none)` fallback → else select the single most-recent matching comment by `createdAt` → parse its finding bullets in BOTH shapes the engine emits — `- <file>:<line> — <title>` (line known) and `- <file> — <title>` (no line) — em-dash `—` separator, one per finding, and render them; the engine's no-findings placeholder bullet `- _(No open findings were recorded.)_` is NOT a finding — when it is the only bullet, render the `(none)` fallback. **No subagent is spawned** — the findings come from the engine's issue comment, not a cluster-side analyzer.
...
**Trigger**: An issue enters `waiting-for:ci` (co-occurs with `agent:paused`; `completed:validate` is ABSENT) — the engine's CI merge gate (`ciMergeGateEnabled`; `phase-loop.ts` `pauseForCiReadiness`) paused `validate` because merge readiness could not open: CI is red (`not-passed`), CI did not turn green within the merge-readiness timeout (`timeout`), or the PR head SHA could not be resolved. The engine states the reason in a plain issue comment on the linked issue (see step 1); the gate record carries no verdict. Verbatim event string: `waiting-for:ci`. The row MUST be recognized so a ci label never falls through to D.10 (unknown-state escalation) — D.10 offers only Skip / Stop, which would strand the issue with no way to re-check CI.

**Source of truth**: Read `to` and `labels` from the enriched doorbell line per § Enriched-line dispatch contract E3 — no per-event `cockpit_status(epic=<epic-ref>, json=true)` re-check on the enriched-line path; bare / malformed lines take the fallback path per FR-005. The ledger row carries `· source: enriched-line` on the enriched-line path (no suffix on fallback) per § Enriched-line dispatch contract E6.

**Dispatch**:
0. **Step 0 — pre-draft gate-status check (UI mode only).** Skip Step 0 entirely under `ResolvedGateMode === "local"`; under `ui`, run the § D.13 Step 0 sub-steps 1–3 byte-identically with `gateType = ci` (generation = PR head SHA per the § UI-mode gate mapping generation-discriminator table) and `dispatchClass: 'D.14'` on every recorded / adopted `openGates` entry. This row's `gateType` maps 1:1 onto this dispatch row, so the § Pre-draft check — shared rules **generation-drift branch guard** is satisfied and the drift branch MAY fire.
1. **Fetch and render the CI verdict from the engine's pause comment.** The engine posts the pause reason as a plain GitHub issue comment on the linked issue (`phase-loop.ts` `pauseForCiReadiness`); the gate record carries no verdict, so the comment is the sole source in BOTH local and UI gate modes. Retrieval: `gh issue view <issue-ref> --json comments` → filter to comments whose `body` `startsWith` the exact, case-sensitive hidden marker `<!-- generacy-ci-pause -->` → **pre-marker engine fallback**: if no comment starts with the marker, filter instead to comments whose `body` `includes` the exact, case-sensitive string `## CI merge readiness paused` → if neither filter matches, render `verdict: (unknown)` → else select the single most-recent matching comment by `createdAt` and render its `CI verdict: <verdict> (source: <check-runs|actions-runs|none>)` line when present (the verdict token is rendered verbatim as the engine states it — e.g. `not-passed`, `pending`, `unknown`; the engine wraps the verdict and head SHA in backticks — strip them when rendering), otherwise (older engines state only a reason sentence — e.g. `CI is red (not-passed) for the head commit; …`, `CI did not turn green within the merge-readiness timeout`, `Could not resolve the PR head commit SHA; …`) render the reason sentence verbatim. Read the PR number and head SHA via `cockpit_context(issue=<issue-ref>)` for the presentation block and the generation discriminator. **No subagent is spawned** — the verdict comes from the engine's issue comment, not a cluster-side CI inspector.
2. **Present CI gate** (see § Gate contract G.10). In one assistant response: the verdict rendering (from step 1) + single `AskUserQuestion` with options `resume (re-check CI)` / `stop` (in that order), header `CI?` (≤ 12 chars), `multiSelect: false`.
3. **Apply verdict**:
   - `resume (re-check CI)` → `cockpit_advance(issue=<issue-ref>, gate="ci")` — applies `completed:ci` server-side; the engine re-arms at `validate` (`phase-resolver.ts` maps `ci` → `{phase: 'validate', resumeFrom: 'validate'}`): it re-runs validate, marks the PR ready, and waits for CI again — a still-red / still-pending CI re-parks the issue at `waiting-for:ci` and this row re-fires (same pattern as D.4's `cockpit_advance(issue, gate="manual-validation")`; `cockpit_resume` is the WRONG verb — it is process/paused-issue resume, not a labeled-gate answer). `auto` does NOT inspect, re-run, or fix CI itself (invariant §1 — anything red is engine-owned).
   - `stop` → exit auto cleanly (kill watch; run summary). **No label writes** (add-only advance invariant §3).

**Ledger line**: `<issue-ref> · waiting-for:ci · ci-gate · <outcome>` — outcomes: `resumed (advanced)` / `advance failed: <desc>` / `stop (exit)`.
...
- `gateKey: string` — the composite `<owner>/<repo>#<issue>:<gateType>:<generation>[:<runId>]`. The trailing `:<runId>` segment is present when the gate was opened under `runIdEnabled === true` and ABSENT under `runIdEnabled === false`. `generation` MAY itself contain colons (`spec-review:<sha>`, `sweep:needs-clarification:2`), so segment detection is **shape-based, not positional**: the issue ref is the prefix before the FIRST `:`, and the runId segment (when present) is the TRAILING colon-free segment matching the runId shape `<tracking-ref-slug>-<timestamp>` (the step-1 no-record guard parses it this way — § D.12 step 1).
...
`issueRef` and `transitionClass` are NOT on the wire — read them from `openGates[event.gateId].issueRef` / `.transitionClass` for the ledger row. **Exception**: the step-1 no-record no-op branches (§ D.12 step 1 foreign-run / out-of-scope) have no `openGates` record to read from, so their ledger `<issue-ref>` slot is the **gateKey-parsed prefix** (before the first `:`) and their transition-class slot is the em-dash placeholder `—`.

**Dispatch steps**:

1. **Look up record**: `record = openGates[event.gateId]`.

   **Record PRESENT** (a current-run entry OR an adopted entry per § step 3 § Adoption pass above) → proceed to steps 2–6 unchanged. The foreign-run / out-of-scope guard below lives ONLY inside the no-record branch and NEVER pre-empts a record match: any answer whose `gateId` matches an `openGates` row is processed by steps 2–6, regardless of which run opened it.

   **Record ABSENT** → classify the delivery before acking. The repo-scoped doorbell replays `gate-answer` history, so a run routinely receives answers belonging to OTHER runs and OTHER epics; a literal `superseded` ack on every no-record delivery would let this run terminate another run's live gate. Parse the `gateKey` (per § Payload shape): the **issue ref** is the prefix before the FIRST `:`; the **runId segment**, when present, is the TRAILING colon-free segment matching the runId shape `<tracking-ref-slug>-<timestamp>`. Detection is **shape-based, NOT positional** — `generation` may itself contain colons (`spec-review:<sha>`, `sweep:needs-clarification:2`), so a fixed segment count would misread it. A gateKey with no runId-shaped trailing segment (a gate opened under `runIdEnabled === false`) has NO runId segment → skip the runId comparison and apply only the in-scope check. Evaluate the three branches in this FIXED order (runId-mismatch is checked BEFORE out-of-scope):

   a. **Foreign-run check (evaluated FIRST)**: the gateKey carries a runId segment AND that segment ≠ this run's pre-flight-derived `runId` (§ step 1 Pre-flight `runId` derivation) → **logged no-op**. Issue NO `cockpit_gate_ack` (no outcome of any kind) and invoke NO downstream handler or dispatch — the gate belongs to another run, which owns its ack. Write exactly ONE ledger row per delivery (replays are NOT deduped — no session-local seen-set, no per-`gateId` or per-wake collapsing): `<gateKey-issue-ref> · — · gate-answer · foreign-run delivery — not acked (owner run: <runId>) · source: ui-gate`. Do NOT dispatch further.

   b. **Out-of-scope check**: the gateKey issue ref ∉ this run's in-scope set (§ step 3 § Adoption pass item 3: `[<epic-ref>] ++ epic.inScopeChildren` under `invocationForm: epic`, or `[<tracking-ref>] ++ trackingIssue.taskListRefs` under the tracking forms) → **logged no-op**, same no-ack / no-downstream rule as branch (a). Write exactly ONE ledger row per delivery (replays NOT deduped): `<gateKey-issue-ref> · — · gate-answer · out-of-scope delivery — not acked (issue: <issue-ref>) · source: ui-gate`. Do NOT dispatch further.

   c. **Fall-through** (same-run, OR no runId segment — a legacy `runIdEnabled === false` gate — AND in scope) → a genuine startup-race or duplicate delivery of THIS run's own gate: `cockpit_gate_ack(gateId, outcome: "superseded", detail: "no matching open record — likely startup-race or duplicate delivery")` — under `runIdEnabled === true` this call ALSO passes the run's pre-flight-derived `runId` verbatim for envelope symmetry with `cockpit_gate_open`, per § step 1 Pre-flight `runId` derivation (per FR-005 / R11; the ack targets an existing `gateId` and performs no key derivation, so this no-record ack lands correctly even when `event.gateId` was minted by a different run); under `runIdEnabled === false` the `runId` field is OMITTED (V6). Write ledger row: `<issue-ref> · <transition-class> · <original-action> · superseded (no record) · source: ui-gate`. Do NOT dispatch further.
...
**Presentation** (same response as the `AskUserQuestion` call) — G.8 has NO findings artifact on either the post-validate or legacy path (the on-ci-green implementation-review branch posts no comment, `phase-loop.ts` on-ci-green branch), so it renders `(none)` unconditionally, keeping exactly the single body row `| (none) | | | |`. No findings-table-from-JSON regeneration and no gate-body parse:
...
**Presentation** (same response as the `AskUserQuestion` call) — the remaining findings fetched and rendered per the D.13 step-1 contract: `gh issue view <issue-ref> --json comments` → select the single most-recent comment (by `createdAt`) whose `body` `startsWith` the exact, case-sensitive marker `<!-- generacy-remediation-limit -->` (pre-marker engine fallback: `includes` the exact, case-sensitive string `## Remediation limit reached`; NEVER `startsWith` the heading — the engine's marker is line 1, the heading line 2, `phase-loop.ts` `REMEDIATION_LIMIT_MARKER` body builder (on-remediation-limit gate)) → parse its `- <file>:<line> — <title>` and `- <file> — <title>` bullets (em-dash `—`; the `- _(No open findings were recorded.)_` placeholder is not a finding) → render `(none)` when no comment matches or only the placeholder bullet is present. Identical in local and UI gate modes (the source is the engine's issue comment, not the gate record). **No subagent** — the findings come from the engine's issue comment, not a cluster-side analyzer:

```markdown
Remediation limit reached for <issue-ref> (PR <pr-number>):

<remaining findings rendered from the `<!-- generacy-remediation-limit -->` / `## Remediation limit reached` comment>
```
...
### G.10 — CI merge-readiness gate

**Trigger**: D.14 (`waiting-for:ci`) — the engine's CI merge gate paused `validate` because CI is not green (red, timed out, or unresolvable head SHA); the verdict lives in a plain issue comment on the linked issue, not in the gate record.

**Presentation** (same response as the `AskUserQuestion` call) — the verdict fetched and rendered per the D.14 step-1 contract: `gh issue view <issue-ref> --json comments` → select the single most-recent comment (by `createdAt`) whose `body` `startsWith` the exact, case-sensitive hidden marker `<!-- generacy-ci-pause -->` (pre-marker engine fallback: `includes` the exact, case-sensitive string `## CI merge readiness paused`) → render its `CI verdict: <verdict> (source: <check-runs|actions-runs|none>)` line, or the reason sentence verbatim when no verdict line is present → render `verdict: (unknown)` when no comment matches. Identical in local and UI gate modes (the source is the engine's issue comment, not the gate record). **No subagent** — the verdict comes from the engine's issue comment, not a cluster-side CI inspector:

```markdown
CI merge readiness paused for <issue-ref> (PR <pr-number>, head <sha>):

CI verdict: <verdict> (source: <check-runs|actions-runs|none>)
<reason sentence from the engine's comment>
```

**Gate invocation** (per § AskUserQuestion invocation contract — one call per CI gate): question `Re-check CI on <issue-ref>?` · header `CI?` (≤ 12 chars) · `multiSelect: false` · exactly two discrete options in order: `resume (re-check CI)` / `stop`.

**Post-gate behavior**:
- `resume (re-check CI)` → `cockpit_advance(issue=<issue-ref>, gate="ci")` — applies `completed:ci` server-side; the engine re-arms at `validate` (re-runs validate, marks the PR ready, waits for CI again; re-parks at `waiting-for:ci` if still not green). Same pattern as D.4's `cockpit_advance(issue, gate="manual-validation")`; `cockpit_resume` is the WRONG verb. `auto` never inspects, re-runs, or fixes CI itself (invariant §1).
- `stop` → exit auto cleanly (kill watch; run summary). **No label writes** (add-only advance invariant §3).

## UI-mode gate mapping (G.1–G.10)

Applies only when `ResolvedGateMode === "ui"` (from § step-1 `--gates` resolution). Under `local`, every gate presents via `AskUserQuestion` per § Gate contract above and this section is dead prose. Under `ui`, every gate contract G.1–G.10 that maps to a per-issue wire record opens a remote gate via `cockpit_gate_open(GateOpenParams)` instead of `AskUserQuestion`; the operator's answer arrives as a D.12 `gate-answer` event and D.12 routes `{optionId, freeText}` onto the SAME downstream handling the local `AskUserQuestion` path performs today — no new downstream behavior.

**`runId` — compute-once, threaded as an explicit literal, propagated to gate-verb-issuing subagents (per #469 / FR-014 / FR-015 / FR-016 / R8).** Under `runIdEnabled === true`, EVERY UI-mode `cockpit_gate_open` invocation in a drafting D.n row (D.1 clarification, D.2 artifact-review, D.3 implementation-review, D.4 manual-validation, D.7 G.4b escalation, D.8 G.5 phase-queue, D.10 G.4c escalation, D.11 G.4d escalation, D.13 remediation-limit, D.14 ci) passes the run's pre-flight-derived `runId` (per § step 1 Pre-flight `runId` derivation and § In-memory loop state additions) VERBATIM on the payload. Also carrying `runId` under `runIdEnabled === true`: the § step 3 startup sweep's `cockpit_gate_open` calls (every extended-trigger row), the § UI-mode fallback branch's local counterpart (wire shape N/A), Form 3's G.6 filing gate open under the TENTATIVE UI window, and every G.5 `phase-queue` open (including the synthetic `phase-bootstrap` variant). Under `runIdEnabled === false` the `runId` field is OMITTED from every payload (V6). No `runId` column is added to the mapping-table rows below because `runId` is per-run, NOT per-gateType — the same value is passed to every open call in the run.
...
**Row count**: EXACTLY 12 rows below — G.1, G.2, G.3, G.4b, G.4c, G.4d, G.5, G.6, G.7, G.8, G.9, G.10 — never including G.4a (D.6 no longer opens a gate — it is ledger-only; the red-validate remediate loop is engine-owned) or G.4e. G.4(e) escalation stays local-only — the per-epic in-memory cursor-fault has no `<issue-ref>` to key on, so the wire record's per-issue fields (`issueRef`, `issueTitle`, `issueUrl`, `branch`) cannot be populated for it; see § G.4(e) exclusion note below.
...
| G.9 | `waiting-for:remediation-limit` | `Remediation limit for <issue-ref>` | Engine's remaining findings fetched from the linked-issue `<!-- generacy-remediation-limit -->` / `## Remediation limit reached` comment (per § D.13 / § G.9 above) — **no subagent** | `resume-remediation` → `resume remediation`; `stop` → `stop` | `{ kind: "none" }` | `resume-remediation`: `cockpit_advance(issue=<ref>, gate="remediation-limit")` (resets the engine remediation counter server-side). `stop`: exit run cleanly (no label writes). | `remediation-limit-gate` |
| G.10 | `waiting-for:ci` | `CI merge readiness for <issue-ref>` | Engine's CI verdict + reason fetched from the linked-issue `<!-- generacy-ci-pause -->` / `## CI merge readiness paused` comment (per § D.14 / § G.10 above) — **no subagent** | `resume-recheck-ci` → `resume (re-check CI)`; `stop` → `stop` | `{ kind: "none" }` | `resume-recheck-ci`: `cockpit_advance(issue=<ref>, gate="ci")` (applies `completed:ci`; the engine re-arms at `validate`). `stop`: exit run cleanly (no label writes). | `ci-gate` |
...
| D.14 ci | `ci-gate` | `resumed (advanced)`, `advance failed: <description>`, `stop (exit)` |
...
| D.12 gate-answer (foreign-run no-op — step 1 no-record, runId mismatch) | `gate-answer` (transition slot is `—`) | `foreign-run delivery — not acked (owner run: <runId>) · source: ui-gate` |
| D.12 gate-answer (out-of-scope no-op — step 1 no-record, issue ∉ scope) | `gate-answer` (transition slot is `—`) | `out-of-scope delivery — not acked (issue: <issue-ref>) · source: ui-gate` |
...
**Exception — step-1 no-record no-op rows.** The D.12 step-1 foreign-run / out-of-scope no-op branches (§ D.12 step 1) do NOT resolve a tracked gate — no `openGates` record exists — so they carry the `gate-answer` action, the em-dash placeholder `—` in the transition slot, and one of the two no-op-specific UI outcomes (`foreign-run delivery — not acked (owner run: <runId>)`, `out-of-scope delivery — not acked (issue: <issue-ref>)`), still with `· source: ui-gate`. These are the ONLY D.12 rows whose `<issue-ref>` slot is the gateKey-parsed prefix rather than `openGates[gateId].issueRef`, and the ONLY ones written without a `cockpit_gate_ack`. Exactly one row per delivery — replays are NOT deduped.
