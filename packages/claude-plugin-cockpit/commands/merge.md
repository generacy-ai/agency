---
description: Merge a PR via the cockpit_merge MCP tool; on red checks, spawn a bounded fixer subagent and re-evaluate. Never merges on red.
arguments:
  - name: issue
...
1. **Parse arguments** — Extract:
   - `<issue>` (positional, optional) — an issue reference (owner/repo#N, #N, or bare integer). If omitted, resolve the current branch's open PR via `gh pr view --json url,number,headRefName -q .` and pass its linked issue to the tool. Passing a PR reference directly is prohibited — the tool resolves the linked PR from the issue internally (see agency#398).
...
2. **Pre-flight** — `command -v gh >/dev/null 2>&1` and `command -v git >/dev/null 2>&1` (the sole remaining Bash CLIs in this playbook: `gh` for PR resolution and `git` for local fixer operations). If either pre-flight returns non-zero, apply the **Error handling** block below with class `MISSING_BINARY` and stop.
...
5. **Decision tree** — Route strictly on `result` + `reason` from the tool return:
   ...
   | unknown `result` or `reason` | — | Print `Stopped: unknown tool result — <result>/<reason>` and exit non-zero. |
...
7. **Spawn fixer subagent** — Print `Spawning fixer (attempt <n>/<max-fix-attempts>)`, then spawn a subagent via the Task tool with `subagent_type: "general-purpose"` and a prompt that:
   ...
   - MUST NOT call `cockpit_merge` itself — this command owns the loop.

8. **Re-evaluate** — When the fixer returns, print `Fixer returned; re-evaluating…` and loop back to step 4 (re-invoke the tool). Never cache the previous tool return; observe state fresh on every call.

9. On any non-zero Bash CLI exit (from `gh` / `git`) or unhandled MCP tool typed error, apply the **Error handling** block below.
...
- **MISSING_BINARY** — pre-flight `command -v gh` or `command -v git` returned non-zero (a required CLI — `gh` for PR resolution, `git` for local fixer operations — is not installed). Print: `A required CLI (\`gh\` for PR resolution, \`git\` for local fixer operations) is required but is not on $PATH. In a Generacy cluster session it is already installed — add it to your PATH: \`export PATH="/shared-packages/node_modules/.bin:$PATH"\` (persist it in ~/.bashrc). Standalone: install both via your platform's package manager (e.g., \`brew install gh git\`).`
...
5. **No direct merge.** MUST NOT call `gh pr merge` or any other merge primitive — only the `cockpit_merge` MCP tool merges.
...
`/cockpit:merge generacy-ai/agency#789 --max-fix-attempts=2` — merges the PR linked to issue #789 with up to two fixer passes. (The bare `#789` is an issue reference, per the tool's contract; the tool resolves its linked PR internally.)
