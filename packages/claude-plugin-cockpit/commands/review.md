...
5. **Post feedback on `request-changes`** — Only when the user selects `request-changes` AND the gate is `implementation-review`. Run the four-step guardrail below. The exact JSON body shape, GraphQL query, marker string, ledger templates, and combined-verdict formula live in the contract docs at `../../../specs/422-summary-auto-md-s/contracts/request-changes-post.md` and `../../../specs/422-summary-auto-md-s/contracts/postcondition-check.md` (source of truth); the prose here names the steps and never restates those shapes verbatim.
   ...
   3. **POST** — `gh api -X POST /repos/<owner>/<repo>/pulls/<pull_number>/reviews --input <bundle>` via the Bash tool; on non-zero exit, apply Error handling. On exit 0, capture `.id`, `.submitted_at`, and `.comments[].length` from the response.
   4. **Verify (two legs)** per `contracts/postcondition-check.md` § Combined verdict:
      - **Leg 1** — `response.comments.length == bundle.comments.length` (the anchored count; the POST accepted every entry).
      - **Leg 2** — run the `reviewThreads(first:50)` GraphQL query from the same contract; filter client-side to nodes matching ALL of: `isResolved == false`, `comments.nodes[0].author.login == <acting-bot-login>` (the `viewer.login` seen by `gh api graphql -f query='{ viewer { login } }'` in this session), `comments.nodes[0].createdAt >= response.submitted_at`. Filtered count MUST be `≥ bundle.comments.length`.
