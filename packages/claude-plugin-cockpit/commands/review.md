...
5. **Post feedback on `request-changes`** — Only when the user selects `request-changes` AND the gate is `implementation-review`. Run the four-step guardrail below. The exact JSON body shape, GraphQL query, marker string, ledger templates, and combined-verdict formula live in the contract docs at `../../../specs/422-summary-auto-md-s/contracts/request-changes-post.md` and `../../../specs/422-summary-auto-md-s/contracts/postcondition-check.md` (source of truth); the prose here names the steps and never restates those shapes verbatim.

   1. **Pre-validate anchors** — fetch the PR diff with `gh pr diff <owner>/<repo>#<pull_number>`, parse `@@ -A,B +C,D @@` hunk headers, and assign each finding an `AnchorCheck` verdict per the data-model rule: `anchored` iff `finding.line != null` AND some hunk in the same file's `[headStart, headStart + headCount − 1]` range contains `finding.line`; everything else is `unanchored` (reason `analyzer-supplied-null` when `finding.line == null`, else `outside-diff-hunks`).
   2. **Compose the POST bundle** per `contracts/request-changes-post.md` § POST body — `event: COMMENT` (NOT `REQUEST_CHANGES`; GitHub blocks `REQUEST_CHANGES` on one's own PR; the Generacy cluster's single-credential model requires `COMMENT`); one `comments[]` entry per anchored finding (`path`, `line`, `body: <summary> — <failure_scenario>`, no `side`, no `start_line`); unanchored findings render into `body` under the literal marker `<!-- generacy-cockpit:unanchored-findings -->` immediately followed by the H2 `## General findings (no file anchor)`, per contract § Unanchored-block shape. **Refuse to POST when `comments.length == 0` AND unanchored count == 0** — request-changes on zero findings is a contract violation; apply Error handling class `OTHER`.
   3. **POST** — `gh api -X POST /repos/<owner>/<repo>/pulls/<pull_number>/reviews --input <bundle>` via the Bash tool; on non-zero exit, apply Error handling. On exit 0, capture `.id` and `.submitted_at` from the response. The POST response body does NOT carry a `comments` field (see `specs/429-re-filed-from-generacy/data-model.md § PostReviewResponse`); Leg 1 counts inline comments via a separate REST endpoint.
   4. **Verify (two legs)** per `contracts/postcondition-check.md` § Combined verdict:
      - **Leg 1** — see `contracts/postcondition-check.md § Leg 1` for the paginated GET-and-filter procedure (single source of truth): `gh api --paginate /repos/<owner>/<repo>/pulls/<pull_number>/comments?per_page=100`, filter to `pull_request_review_id == response.id`, early-exit at `bundle.comments.length`, inline poll 500 ms → 1 s → 2 s across four attempts. `Leg1Check.outcome == "genuine-undercount"` (never `undefined.length`) is the failure signal that trips the outer 2 s → re-POST retry.
      - **Leg 2** — run the `reviewThreads(first:50)` GraphQL query from the same contract; filter client-side to nodes matching ALL of: `isResolved == false`, `comments.nodes[0].author.login == <acting-bot-login>` (the `viewer.login` seen by `gh api graphql -f query='{ viewer { login } }'` in this session; both sides under `postcondition-check.md § Login normalization`), `comments.nodes[0].createdAt >= response.submitted_at`. Filtered count MUST be `≥ bundle.comments.length`.
      Success ⇔ both legs pass. On failure: sleep 2000 ms, retry the POST once with the same bundle; if the second attempt's postcondition also fails, hand back to the caller's verdict-gate re-presentation (in `auto.md` D.2/D.3 this is the G.2 re-presentation shape; in the standalone `/cockpit:review` invocation this exits with the failure notice on stdout and non-zero exit, so the operator can decide manually). Do NOT emit the `Feedback posted:` line on failure.
   5. **On success** (first attempt or after retry): print one line `Feedback posted: N inline comment(s) on PR #<pull_number>` where `N` is the anchored-finding count (`bundle.comments.length`) — this success line is the Terminal Outcome Check marker for the `request-changes` branch and MUST be emitted ONLY after BOTH postcondition legs pass. Do NOT call `cockpit_advance`. Do NOT emit a `Labels:` line. <!-- The unresolved review threads posted by this step trip PrFeedbackMonitorService, which applies waiting-for:address-pr-feedback and enqueues fix work. That handler owns the label transition; this command must not race it. -->
...
- **request-changes** — Step 5 executed and emitted the success line matching `Feedback posted: N inline comment(s) on PR #<pull_number>` **after both postcondition legs passed** (`N` is the anchored-finding count). Absence of this line is now a hard signal that the guardrail failed and the verdict gate was re-presented (or, in standalone invocation, the command exited non-zero with the failure notice on stdout) — not a soft/best-effort miss.
- **request-changes (re-presented gate)** — When the D.2/D.3 caller re-presents G.2 after a retried postcondition failure, the terminal outcome for this invocation is whatever verdict the operator selects on the re-presentation (a fresh `approve` / `request-changes` / `abort`). The re-presented gate's own terminal marker (the new selection's `Labels:` / `Feedback posted:` / `Aborted:` line) covers the outcome; this invocation's Terminal Outcome Check is satisfied by handing control back to the re-presenter.
